...
Каждая запись имеет метку (label), произвольную текстовую метаданные и версию.
Версия используется для оптимистичной блокировки: обновление, отправленное на
основе устаревшей версии, отклоняется конфликтом. Данные одного пользователя
...
Суммарное покрытие кода тестами — **80%+** (цель по ТЗ — не менее 70%).
Покрытие измеряется по всем пакетам, включая сгенерированный protobuf-код;
пакет `internal/common/proto` покрыт контрактными тестами (round-trip
сообщений и проверка RPC-обвязки поверх bufconn).
