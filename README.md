...
- **обязательный TLS** между клиентом и сервером — plaintext-соединения не
  поддерживаются ни клиентом, ни сервером.

### TLS на сервере (один из двух режимов)

**Режим 1 — самоподписанные сертификаты (локальная разработка, self-hosted):**

```bash
# 1. Сгенерировать серты (сан-адреса должны покрывать адрес клиента)
gophkeeper-server gen-cert --out-dir ./certs --dns localhost --ip 127.0.0.1

# 2. Запустить сервер
gophkeeper-server --tls-cert ./certs/server.crt --tls-key ./certs/server.key \
  --dsn postgres://... --jwt-secret ...
```

**Режим 2 — автоматические сертификаты Let's Encrypt (публичный домен):**

```bash
gophkeeper-server --autocert-domain=gk.example.com --autocert-cache-dir ./certs-cache \
  --dsn postgres://... --jwt-secret ...
```

Требования autocert-режима: публичное доменное имя, доступность портов 443/80
для ACME-челленджа (TLS-ALPN-01/HTTP-01).

### TLS на клиенте

- `--tls-ca=<файл>` — доверять серверу по указанному CA-сертификату
  (self-signed сценарий: укажите `server.crt` из gen-cert). То же через
  переменную `GOPHKEEPER_CA_PATH`.
- без флага — сервер проверяется по системным корневым CA (сценарий
  Let's Encrypt/autocert или сертификат публичного CA).
