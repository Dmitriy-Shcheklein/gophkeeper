...
var (
	...
	// ErrInvalidEntryID is returned when the entry identifier is not a
	// well-formed UUID: entry ids are generated by the database and
	// are always UUIDs, so a malformed id can never match a stored
	// entry and must not reach the repository (a raw SQL query with it
	// fails with a driver-level error instead of a clean not-found).
	ErrInvalidEntryID = errors.New("entry id must be a valid UUID")
...
// validateEntryID checks the entry identifier: it must be non-empty
// and a well-formed UUID. Validating at the service boundary keeps
// malformed ids from reaching the database, where they would fail as
// driver-level errors instead of clean domain errors.
